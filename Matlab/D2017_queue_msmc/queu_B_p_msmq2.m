...
Pointer=2;
for i=3:arr_num
    if ba4_a(length(member_A_b))<=ba4_b(length(member_B_b))
        ...
        number=sum(ba3_a(1:len_A_mem_b-1)>=ba1_a(len_A_mem_b));
         ...
         number=sum(ba3_b(1:len_B_mem_b-1)>=ba1_b(len_B_mem_b));
          ...
          bottleB(rem(Pointer+1,10)+1)=ba2_a(len_B_mem_b)-ba1_a(len_B_mem_b);
          Pointer=Pointer+1;
    end
    if (MODEL==1)
...
len_A_mem_b = length(member_A_b);
len_B_mem_b = length(member_B_b);
b1=[ba1_a(1:len_A_mem_b) ba1_b(1:len_B_mem_b)];
b2=[ba2_a(1:len_A_mem_b) ba2_b(1:len_B_mem_b)];
b3=[ba3_a(1:len_A_mem_b) ba3_b(1:len_B_mem_b)];
b4=[ba4_a(1:len_A_mem_b) ba4_b(1:len_B_mem_b)];
b_tmp=[b1;b2;b3;b4];
...
da1=zeros(1,sum(ba5_a)+sum(ba5_b));
index=find(ba5_a==1);
for i=1:sum(ba5_a)
    da1(i)=ba3_a(index(i));
end
