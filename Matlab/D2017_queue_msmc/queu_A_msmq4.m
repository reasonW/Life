...
state_c=abs(state_c);
state_d=normrnd(12.551,4.54,1,arr_num);
state_d=abs(state_d);
...
a1_d=zeros(1,arr_num); %每个人到达时间
a2_d=zeros(1,arr_num); %每个人开始接受服务时间   -> events(3,:)
a3_d=zeros(1,arr_num);%每个人离开时间            -> events(4,:)
a4_d=zeros(1,arr_num);% %标识位表示其进入系统后，系统内共有的顾客数       -> events(5,:)
...
a1_d(1)=arr_time(3);%第1个人到达时间
a2_d(1)=arr_time(3);%第1个人开始服务时间为到达时间
a3_d(1)=a2_d(1)+state_d(1); %第1个人离开时间
a4_d(1)=1;%一个顾客,标志位置为1
...
Pointer=4;
for i=5:arr_num
    if a4_a(length(member_A)) == min( a4_d(length(member_D)), min(a4_a(length(member_A)),min(a4_b(length(member_B)),a4_c(length(member_C)))))
        ...
        number=sum(a3_a(1:len_A_mem-1)>=a1_a(len_A_mem));
          ...
          Pointer=Pointer+1;
    elseif  a4_b(length(member_B)) == min( a4_d(length(member_D)), min(a4_a(length(member_A)),min(a4_b(length(member_B)),a4_c(length(member_C)))))
         ...
         number=sum(a3_b(1:len_B_mem-1)>=a1_b(len_B_mem));
          ...
          Pointer=Pointer+1;
    elseif  a4_c(length(member_C)) == min( a4_d(length(member_D)), min(a4_a(length(member_A)),min(a4_b(length(member_B)),a4_c(length(member_C)))))
         ...
         member_D=[member_D,length(member_D)+1];
         len_D_mem=length(member_D);
         a1_d(len_D_mem)=arr_time(i);
         number=sum(a3_b(1:len_D_mem-1)>=a1_d(len_D_mem));
          if number ==0
            a2_d(len_D_mem)=a1_d(len_D_mem);
            a3_d(len_D_mem)=a1_d(len_D_mem)+state_c(len_D_mem);
            a4_d(len_D_mem)=1;
          else
            a2_d(len_D_mem)=a3_d(len_D_mem-1);
            a3_d(len_D_mem)=a2_d(len_D_mem)+state_c(len_D_mem);
            a4_d(len_D_mem)=number+1;
          end
          bottleA(rem(Pointer+1,10)+1)=a2_b(len_D_mem)-a1_b(len_D_mem);
          Pointer=Pointer+1;
    end
    if (MODEL==1)
...
len_C_mem = length(member_C);
len_D_mem = length(member_D);
a1=[a1_a(1:len_A_mem) a1_b(1:len_B_mem) a1_c(1:len_C_mem) a1_d(1:len_D_mem)];
a2=[a2_a(1:len_A_mem) a2_b(1:len_B_mem) a2_c(1:len_C_mem) a2_d(1:len_D_mem)];
a3=[a3_a(1:len_A_mem) a3_b(1:len_B_mem) a3_c(1:len_C_mem) a3_d(1:len_D_mem)];
a4=[a4_a(1:len_A_mem) a4_b(1:len_B_mem) a4_c(1:len_C_mem) a4_d(1:len_D_mem)];
a_tmp=[a1;a2;a3;a4];
