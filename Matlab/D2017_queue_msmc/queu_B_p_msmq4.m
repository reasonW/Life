...
state_c_b=zeros(1,arr_num_b);state_d_b=zeros(1,arr_num_b);
tmp_a_b=normrnd(232.39,143.94,1,arr_num_b+1);
tmp_a_b=sort(abs(tmp_a_b));
for i=1:arr_num_b
    if(MODEL==1)
        state_a_b(i)=(tmp_a_b(i+1)-tmp_a_b(i))*1/7;% pre check
    else
...
tmp_d_b=normrnd(232.39,143.94,1,arr_num_b+1);
tmp_d_b=sort(abs(tmp_d_b));
for i=1:arr_num_b
    if(MODEL==1)
         state_d_b(i)=(tmp_d_b(i+1)-tmp_d_b(i))*1/7;% pre check
    else
         state_d_b(i)=(tmp_d_b(i+1)-tmp_d_b(i))*1;% Regular
    end
end
...
ba1_d=zeros(1,arr_num_b); %每个人到达时间
ba2_d=zeros(1,arr_num_b); %每个人开始接受服务时间   -> events(3,:)
ba3_d=zeros(1,arr_num_b);%每个人离开时间            -> events(4,:)
ba4_d=zeros(1,arr_num_b);% %标识位表示其进入系统后，系统内共有的顾客数       -> events(5,:)
ba5_d=zeros(1,arr_num_b);% 标识位是否进入复查
...
ba1_d(1)=arr_time_b(2);%第1个人到达时间
ba2_d(1)=arr_time_b(2);%第1个人开始接受服务时间为到达时间
ba3_d(1)=ba2_d(1)+state_d_b(1); %第1个人离开时间
ba4_d(1)=1;%一个顾客,标志位置为1
ba5_d(1)=randsrc(1,1,[0 1;aerfa 1-aerfa]);
...
for i=4:arr_num
    if ba4_a(length(member_A_b))== min( ba4_d(length(member_D_b)),min(ba4_a(length(member_A_b)),min(ba4_b(length(member_B_b)),ba4_c(length(member_C_b)))))
        ...
        number=sum(ba3_a(1:len_A_mem_b-1)>=ba1_a(len_A_mem_b));
          ...
          Pointer=Pointer+1;
    elseif ba4_b(length(member_B_b))== min( ba4_d(length(member_D_b)),min(ba4_a(length(member_A_b)),min(ba4_b(length(member_B_b)),ba4_c(length(member_C_b)))))
         ...
         number=sum(ba3_b(1:len_B_mem_b-1)>=ba1_b(len_B_mem_b));
          ...
          Pointer=Pointer+1;
    elseif ba4_c(length(member_C_b))== min( ba4_d(length(member_D_b)),min(ba4_a(length(member_A_b)),min(ba4_b(length(member_B_b)),ba4_c(length(member_C_b)))))
         ...
         member_D_b=[member_D_b,length(member_D_b)+1];
         len_D_mem_b=length(member_D_b);
         ba1_d(len_D_mem_b)=arr_time_b(i);
         number=sum(ba3_d(1:len_D_mem_b-1)>=ba1_d(len_D_mem_b));
          if number ==0
            ba2_d(len_D_mem_b)=ba1_d(len_D_mem_b);
            ba3_d(len_D_mem_b)=ba1_d(len_D_mem_b)+state_c_b(len_D_mem_b);
            ba4_d(len_D_mem_b)=1;
            ba5_d(len_D_mem_b)=randsrc(1,1,[0 1;1-aerfa aerfa]);
          else
            ba2_d(len_D_mem_b)=ba3_d(len_D_mem_b-1);
            ba3_d(len_D_mem_b)=ba2_d(len_D_mem_b)+state_c_b(len_D_mem_b);
            ba4_d(len_D_mem_b)=number+1;
            ba5_d(len_D_mem_b)=randsrc(1,1,[0 1;1-aerfa aerfa]);
          end
          bottleB(rem(Pointer+1,10)+1)=ba2_d(len_D_mem_b)-ba1_d(len_D_mem_b);
          Pointer=Pointer+1;
    end
    if (MODEL==1)
...
len_C_mem_b = length(member_C_b);
len_D_mem_b = length(member_D_b);
b1=[ba1_a(1:len_A_mem_b) ba1_b(1:len_B_mem_b) ba1_c(1:len_C_mem_b) ba1_d(1:len_D_mem_b)];
b2=[ba2_a(1:len_A_mem_b) ba2_b(1:len_B_mem_b) ba2_c(1:len_C_mem_b) ba2_d(1:len_D_mem_b)];
b3=[ba3_a(1:len_A_mem_b) ba3_b(1:len_B_mem_b) ba3_c(1:len_C_mem_b) ba3_d(1:len_D_mem_b)];
b4=[ba4_a(1:len_A_mem_b) ba4_b(1:len_B_mem_b) ba4_c(1:len_C_mem_b) ba4_d(1:len_D_mem_b)];
b_tmp=[b1;b2;b3;b4];
...
if sum(ba5_a)+sum(ba5_b)+sum(ba5_c)+sum(ba5_d)==0
    ba5_a(1)=1;
end

da1=zeros(1,sum(ba5_a)+sum(ba5_b)+sum(ba5_c)+sum(ba5_d));
index=find(ba5_a==1);
for i=1:sum(ba5_a)
    da1(i)=ba3_a(index(i));
end
...
index=find(ba5_d==1);n=1;
for i=sum(ba5_a)+sum(ba5_b)+sum(ba5_c)+1:sum(ba5_a)+sum(ba5_b)+sum(ba5_c)+sum(ba5_d)
    da1(i)=ba3_d(index(n));
    n=n+1;
end
da1=sort(da1);
